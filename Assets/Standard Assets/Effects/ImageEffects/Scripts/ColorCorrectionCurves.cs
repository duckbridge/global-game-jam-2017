...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class ColorCorrectionCurves : PostEffectsBase
	{
		...
		private bool isChangingSaturationIncremental = false;
		private float saturationTarget = 0f;
		private float saturationIncrementValue = 0f;
		...
		public void FixedUpdate() {
			...
			if(isChangingSaturationIncremental) {
				if(Mathf.Abs (saturationTarget - saturation) > .01f) {
					saturation += saturationIncrementValue;
				} else {
					saturation = saturationTarget;
					isChangingSaturationIncremental = false;
				}
			}
		}

		public void ChangeSaturationTo(float saturationTarget, float saturationIncrementValue) {

			if(saturationTarget > saturation) {
				saturationIncrementValue = Mathf.Abs(saturationIncrementValue);
			} else {
				saturationIncrementValue = -Mathf.Abs(saturationIncrementValue);
			}

			this.saturationTarget = saturationTarget;
			this.saturationIncrementValue = saturationIncrementValue;

			isChangingSaturationIncremental = true;
		}

		public void StopChangingSaturation() {
			isChangingSaturationIncremental = false;
		}
